...
export class ChatService {
  ...
  async createDM(payload: CreateChatSocketDto): Promise<UpdateChatDto> {
    console.log(
      `creating chat... for ${payload.user1Id} and ${payload.user2Id}`,
    );
    ...
    console.log(
      `Added users ${payload.user1Id} and ${payload.user2Id} to chat ${newChat.id}`,
    );
